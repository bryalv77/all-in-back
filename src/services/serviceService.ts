...
export default class ServiceService {
  ...
  async create(data) {
    ...
    try {
      ...
      const record = await ServiceRepository.create(data, {
        ...this.options,
        transaction,
      });
  ...
  async update(id, data) {
    ...
    try {
      ...
      const record = await ServiceRepository.update(
        id,
        data,
        {
          ...this.options,
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await ServiceRepository.destroy(id, {
          ...this.options,
          transaction,
        });
      }
  ...
  async findById(id) {
    return ServiceRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return ServiceRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return ServiceRepository.findAndCountAll(
      args,
      this.options,
    );
  }
