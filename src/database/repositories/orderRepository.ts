...
class OrderRepository {
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const include = [
      ...
      {
        model: options.database.deliveryMethod,
        as: 'deliver',
      },
    ];
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.shippingPriceRange) {
        const [start, end] = filter.shippingPriceRange;
      ...
      if (filter.deliver) {
        whereAnd.push({
          ['deliverId']: SequelizeFilterUtils.uuid(
            filter.deliver,
          ),
        });
      }
