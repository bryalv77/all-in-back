...
class CourseRepository {
    ...
    const record = await options.database.course.create(
      {
        ...
        teacherIdId: data.teacherId || null,
        tenantId: tenant.id,
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await options.database.course.findOne(      
      {
        where: {
          id,
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const include = [
      {
        model: options.database.teacher,
        as: 'teacherId',
      },
    ];
    ...
    const record = await options.database.course.findOne(
      {
        where: {
          id,
          tenantId: currentTenant.id,
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.teacherId) {
        whereAnd.push({
          ['teacherIdId']: SequelizeFilterUtils.uuid(
            filter.teacherId,
          ),
        });
      }
